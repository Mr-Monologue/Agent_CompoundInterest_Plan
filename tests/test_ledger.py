...
def test_new_portfolio_has_no_implicit_allocation_policy(
    tmp_path: Path,
) -> None:
    ...
    with pytest.raises(LedgerError) as captured:
        service.get_allocation_policy(portfolio_id=portfolio_id)
    assert captured.value.code == "ALLOCATION_POLICY_NOT_CONFIGURED"
