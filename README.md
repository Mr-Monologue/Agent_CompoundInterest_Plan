...
0.27.0 将覆盖快照中的有界请求升级为可审计运行任务。`research_collection_task_build` 只从
一个明确的不可变覆盖快照生成幂等任务；`research_collection_task_claim` 仅允许当前启用且
能力匹配的连接器获得最长 60 分钟租约，并限制为最多三次尝试。租约令牌只在认领时返回，
Core 只保存摘要；重复认领、过期租约、来源谱系越界和任务范围外证据都会被稳定错误码拒绝。

`research_collection_task_result_record` 复用既有采集批次和研究证据入口保存逐项结果，失败
回执不创建占位证据；成功或部分成功后按原快照的标的、证据类型、日期和时效阈值重建新覆盖
快照，并把任务、attempt、collection run、前后 coverage snapshot 串成可追溯链。
`research_collection_runtime_status_get` 只报告队列、租约、重试和连接器运行事实。任务完成
不等于来源独立验证，覆盖关闭也不是基金排名、投资建议、策略修改或交易授权。
