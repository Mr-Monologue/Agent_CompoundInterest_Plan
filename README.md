...
0.28.0 将产品入口从大量独立能力收敛为一个只读的 Hermes 投资工作台。
`investment_workspace_get` 的 `DAILY` 视图聚合当前组合、估值质量、运行模式、自动化状态、
待重试或漏跑任务、周计划、卖出提案、复盘行动和研究采集异常，并按固定的操作与人工复核
优先级输出现有事实。优先级只用于组织工作流，不评价基金、不推导投资动作，也不会自动调用
返回的查看工具。

`WEEKLY` 视图以指定数据日为截止日，固定汇总最近 7 个自然日内已经提交的交易账本、现金事实、
周计划、周期复盘和自动化事实包。它只读取既有事实，不生成计划、交易、持仓变化或投资建议。

`READINESS` 视图检查默认上下文、策略实例、估值事实、运行模式、自动化与 Hermes 调度一致性、
真实通知测试、已验证备份、开放告警和 14 天运行窗口；研究连接器保持可选，不阻塞核心投资
助手的 V1 就绪判断。`FULL` 仅合并两个视图。三个视图都要求 Hermes 原样返回 Core 的
...
0.28.2 修复自动化失败告警与重试最终状态脱节的问题。同一个 `job_run_id` 的后续尝试成功
完成后，即使因为数据质量警告而以 `DEGRADED` 收尾，只要执行结果为成功且错误已清空，Core
就会把该运行关联的 `OPEN` 或 `ACKNOWLEDGED` 告警自动标记为 `RESOLVED`。解决记录包含时间、
执行者、`JOB_RUN_RECOVERED` 原因、最终尝试次数和数据质量，并写入独立审计事件；原始失败
上下文、告警次数和通知投递历史保持不变。

告警列表同时返回关联 Job Run 的实时状态，避免把首次失败快照误读为当前状态。迁移
`0025_alert_recovery_resolution` 会按相同条件回填历史过时告警，只处理已成功恢复且没有当前
错误的精确关联运行；仍失败、仍待重试或属于其他时间窗的告警不会被解决。本变更不提供任意
手工清除入口，也不修改持仓、策略、计划或交易。

0.29.0 为卫星舱增加受治理的估值信号门控。用户先通过草稿和确认令牌批准一个组合级
SATELLITE 信号政策，明确 PE/PB 指标、进入候选的最高分位、回看期、最小样本数、最大数据
年龄以及是否允许 WARNING 质量。政策提交不会修改任何标的配置；基金到指数的
`benchmark_code` 和 `proxy_suitability` 仍必须逐只明确确认，Core 不根据基金名称、持仓或
模型意见猜测映射。

`satellite_signal_snapshot_build` 使用已存指数估值证据生成不可变快照。只有显式设为
`contribution_eligible=true`、代理适用性为 `STRONG`、样本与时效满足政策且分位不高于批准
阈值的卫星标的才得到 `OPEN`。无基准、弱代理、样本不足、数据过期、质量受限或未获长期
准入时分别返回确定性阻塞原因。`OPEN` 只代表可进入本期周计划候选，不是排名、推荐或成交。

存在活动信号政策时，周计划要求卫星标的同时满足策略准入和同数据日期、同政策、同策略实例
...
0.29.2 修正 Core 运行时版本报告，使 `/ready`、doctor、包元数据和发布清单保持一致，并增加防回归契约。

0.30.0 增加周计划的分批执行闭环。已冻结计划可以逐笔关联用户在外部平台真实完成、随后经
草稿—确认—提交写入账本的 BUY 记录；首笔未完成全部金额时进入 `PARTIALLY_EXECUTED`，并按
基金显示计划金额、累计成交金额和剩余金额。只有全部基金的账本成交金额精确满足冻结计划后
才进入 `EXECUTED`。同一成交不能重复使用，已冲销、跨组合、跨账户、非 BUY、非计划基金或
超额成交都会被拒绝。累计口径复用账本 `amount`（已由净值和份额校验）；账本没有独立手续费
字段，因此手续费不单独计入计划金额。该能力不会创建交易、不会代替用户买入，也不会修改
冻结计划内容。数据库修订为 `0027_partial_plan_execution`。

0.31.0 增加场外基金申购的提交、在途和份额确认生命周期。用户可以先记录已经在支付宝等外部
平台提交的申购；在平台确认净值和份额之前，该金额只占用冻结计划，不计入持仓或正式成交。
系统支持跨日、跨周和多次部分确认，也支持明确的取消、拒绝和冲销事实。只有用户再次明确确认
后，完整确认事实才会复用现有 BUY 账本，并按“确认本金加手续费”的现金口径累计计划执行金额。
下一周计划会先扣除历史未完成冻结计划，避免重复安排同一笔资金。日报、周报和投资工作台会显示
已成交、在途、未提交及取消或退款金额；超出预计确认日期只提示复核，不自动判断失败。卫星舱
直接定投继续关闭时，不再阻断核心舱最小闭环。数据库修订为
`0028_external_subscription_lifecycle`。该版本不会自动申购、买入、卖出、确认平台状态或修改持仓。
该补丁不改变数据库、策略、计划、交易、持仓或 Hermes 调度。

0.31.3 为已过期且从未提交的外部申购草稿增加显式安全重签。重签沿用原草稿 ID、幂等键、
载荷和载荷摘要，只在事务内轮换确认凭据与到期时间，并记录重签次数和时间；并发请求最多一个
成功。查询接口会把超过到期时间的未提交草稿明确显示为 `EXPIRED`。外部申购 `SUBMIT` 与
`CONFIRM` 草稿的新建和重签默认有效期调整为可配置的 24 小时，既有草稿不会自动延长。
该操作不创建申购、交易、现金、持仓或计划执行事实。数据库修订为
`0031_external_subscription_draft_renewal`。

0.31.4 为外部申购份额确认增加结构化时间精度。来源提供真实时分秒时使用 `EXACT`，并要求
该时刻在业务时区的日期与确认日期一致；来源只提供日期时使用 `DATE_ONLY`，Core 将业务日期
零点保存为内部排序兼容值，但查询、报告和审计同时返回精度，面向用户只显示确认日期。旧客户端
不传精度时继续按 `EXACT` 处理并要求原有的带时区时间戳；历史事实和草稿统一采用兼容默认
`EXACT`，不会根据零点时间猜测来源精度。

本版本同时增加 `external_subscription_confirmation_draft_revise`。它只修订未提交的 CONFIRM
草稿，要求调用方提供最近读取的载荷摘要，通过事务条件更新保留原草稿 ID、原幂等键、所属申购
及外部订单引用，并重新签发短期确认凭据。修订本身不创建确认、交易、现金、持仓或计划执行事实；
只有用户再次明确确认并提交修订后的草稿，正式确认事实才会写入。数据库修订为
`0032_confirmation_time_precision_revision`。

0.31.5 统一外部申购确认转 BUY 交易的成本口径：申购提交金额是用户实际支付毛额，份额确认金额
是扣费后换取份额的净额，手续费保持独立事实。交易金额、持仓成本、派生现金支出和冻结计划执行额
均使用毛额，手续费不会再额外扣除；确认份额、确认净值、净值日期及时间精度保持不变。系统以整数
最小货币单位核对“毛额约等于净额加手续费”，只接受一最小货币单位的既有舍入差异，超出时拒绝
记账并保留确认与持仓原状。

本版本同时增加 `external_subscription_transaction_draft_revise`。它只允许在原确认、原草稿和原
...
过期时，可创建独立、短期有效的关闭草稿；只有再次明确确认该草稿，计划才进入 `SKIPPED`。
旧凭据不会被恢复、展示或绕过。计划列表、工作台和 readiness 统一把 `FROZEN` 解释为仍然有效、
需要用户处理并继续占用后续计划额度，直到计划执行或明确跳过。数据库修订为
`0029_weekly_plan_skip_reconfirmation`。本修复不会自动修改策略、关闭计划、创建申购或执行交易。
...
1. 连接器合规与真实研究运行：契约符合性测试、标准测试夹具、采集运行观测与跨期连续性。
2. 大阶段四：通用初始化向导、干净安装验收、备份恢复和迁移工具。
3. 大阶段四收口：14 天连续运行验收、公共分发文档和 V1 发布候选。
