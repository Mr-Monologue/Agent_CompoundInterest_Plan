...
is unavailable, use `weekly_plan_skip_draft_create`, show the exact close preview, retain its new
short-lived token only in the current confirmation lifecycle, and call
`weekly_plan_skip_draft_commit` only after a fresh explicit confirmation. Never recover, display or
bypass the original credential. Use `weekly_plan_transaction_link` only after the user
...
object before asking for confirmation. This creates no strategy change until exact commit.
